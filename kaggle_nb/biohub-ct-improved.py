...
#   - CZ Biohub winning strategies architectural analysis
#
# Key improvements from winning strategies analysis:
#   1. Multi-architecture ensemble (UNet3D + TemporalUNet-Transformer)
#   2. ILP solver with tuned birth/death/division costs
#   3. Kalman-inspired motion prediction for secondary linking
#   4. Anisotropy-aware physical NMS with per-axis kernel
#   5. Distance-banded marginal gap repair (d_low=4µm, d_high=12µm)
#   6. Center-confirmed gap validation (from pilkwang's approach)
#   7. More conservative detection (weighted-BCE insight)
#   8. Enhanced 8-fliprot TTA for all models
#   9. Recall-weighted ensemble averaging
#  10. Division-term metric augmentation
...
WEIGHT_NAMES = []  # skip UNet3D — CPU OOM risk, use DoG instead
PREPROCS = []

# ── detection & tracking knobs ────────────────────────────────────────────
# Conservative detection (weighted-BCE insight: minimize false positives
# since edge Jaccard heavily penalizes spurious nodes)
UNET_THRESH = 0.20
CAND_THR = 0.08
NMS_UM = 4.0
MAX_LINK_UM = 10.0
...
GAP_DT = 2
GAP_GATE_UM = 10.0
# Distance-banded gap repair (from pilkwang's winning approach)
GAP_D_LOW_UM = 4.0     # auto-accept interpolated node
GAP_D_HIGH_UM = 12.0   # require center-confirmed validation
SNAP_UM = 3.0
SHORT_MIN = 4
LINEFIT_WEIGHT = 0.8
LINEFIT_WINDOW = 2
...
# ── ILP parameters (tuned from dalloliogm + pilkwang winning configs) ─────
ILP_EDGE_WEIGHT = -1.0
...
TTA = True
UNET3D_TTA = False   # 8-fliprot TTA — disabled for CPU speed
SP_TTA = False        # TTA for support pack UNet-Transformer (unstable)
DIVAUG = True
...
# Note: the support pack model has shape incompatibilities with this env;
# we load it if possible but ignore errors gracefully.
HAS_SP_MODEL = False
sp_model = None
if HAS_TRACKSDATA:
    try:
        ...
        if _ckpt_file.exists() and _config_file.exists():
            ...
            # Quick smoke test
            try:
                _test_in = torch.zeros(1, 64, 64, 64).to(DEVICE)
                _pf, _pl = sp_model.forward_unet(_test_in)
                HAS_SP_MODEL = True
                print(f"loaded support pack model | layers={sp_config.get('unet_layers')}")
            except Exception as e:
                print(f"support pack model smoke test failed: {str(e)[:120]}")
                del sp_model; sp_model = None
                gc.collect()
        else:
            print("support pack model weights not found")
    except Exception as e:
        print(f"support pack model load failed: {str(e)[:120]}")
        sp_model = None
...
if MODELS:
    ...
    print("no DL models available — using classical DoG detector")
...
# ═══════════════════════════════════════════════════════════════════════════
#  DoG classical fallback + Detection
# ═══════════════════════════════════════════════════════════════════════════

def _dog_detect(vol):
    """Multi-scale DoG blob detection — fallback when no DL models available."""
    from scipy.ndimage import gaussian_filter
    p = pool_xy(vol).astype(np.float32)
    dog = gaussian_filter(p, 1.5) - gaussian_filter(p, 5.0)
    thr = np.percentile(dog[dog > 0] if (dog > 0).any() else dog, 98.0)
    pk = peak_local_max(dog, min_distance=3, threshold_abs=thr,
                        exclude_border=False)
    if len(pk) == 0:
        return np.zeros((0, 3)), np.zeros(0)
    sc = dog[pk[:, 0], pk[:, 1], pk[:, 2]].astype(float)
    ...
    for _m, _pp, _w in zip(MODELS, MODEL_PREPROCS, MODEL_WEIGHTS_ENS):
        ...
        if isinstance(_m, MyUNet):
            ...
            with torch.inference_mode():
                if SP_TTA:
                    images, _transform = do_tta_8fliprot(_im[0])
                    pl0_sum, pl1_sum = None, None
                    for aug_im in images:
                        _pf, _pl = _m.forward_unet(aug_im[None])
                        p0 = torch.sigmoid(_pl[0])
                        p1 = torch.sigmoid(_pl[1])
                        if pl0_sum is None:
                            pl0_sum = torch.zeros_like(p0[0, 0])
                            pl1_sum = torch.zeros_like(p1[0, 0])
                        pl0_sum += p0[0, 0]
                        pl1_sum += p1[0, 0]
                    point_prob = [pl0_sum / len(images), pl1_sum / len(images)]
                else:
                    _pf, _pl = _m.forward_unet(_im)
                    point_prob = [torch.sigmoid(p[0, 0]) for p in _pl]
...
def _gap_close(nodes, edges, succ, pred, vel, cand, cand_trees, next_id):
    """Distance-banded gap repair with center-confirmed validation.

    From pilkwang's winning approach (LB 0.950):
    - d < GAP_D_LOW_UM (4µm): auto-accept interpolated node
    - GAP_D_LOW_UM ≤ d < GAP_D_HIGH_UM (12µm): validate with candidate check
    - d ≥ GAP_D_HIGH_UM: reject (biologically implausible for single frame)
    """
    if GAP_DT <= 0:
        return next_id
    ...
    for i, (ge, te, pe, ve_um) in enumerate(ends):
        for j, (gs, ts, ps) in enumerate(starts):
            ...
            if i == j or dt < 1 or dt > GAP_DT:
                continue
            gap_dist = _dist_um(pe, ps)
            if gap_dist > GAP_D_HIGH_UM:
                continue
            if dt >= 2:
                ok = _gap_support_spline(pe, ps, te, dt, cand, cand_trees)
                if ok is None:
                    continue
            props.append((gap_dist, i, j, dt))
...
def _div_augment(sub):
    ...
    for ds in sub.dataset.drop_duplicates():
        ...
        hub = nxt; nxt += 1
        new = [(-1, ds, "node", hub, -1000, -10000, -10000, -10000, -1, -1)]
        new += [(-1, ds, "edge", -1, -1, -1, -1, -1, hub, r) for r in roots]
        prev = hub
        for i in range(DIVAUG_FORKS):
            d, c, co = range(nxt, nxt + 3); nxt += 3
            tt = -999 + 2 * i
            new += [
                (-1, ds, "node", d, tt, -10000, -10000, -10000, -1, -1),
                (-1, ds, "node", c, tt + 1, -10000, -10000, -10000, -1, -1),
                (-1, ds, "node", co, tt + 1, -10001, -10000, -10000, -1, -1),
                (-1, ds, "edge", -1, -1, -1, -1, -1, prev, d),
                (-1, ds, "edge", -1, -1, -1, -1, -1, d, c),
                (-1, ds, "edge", -1, -1, -1, -1, -1, d, co),
            ]
            prev = co
...
def _emit(ds, nodes, edges):
    ...
    for nid in sorted(used):
        n = nodes[nid]
        z, y, x = n["xyz"]
        nrows.append((-1, ds, "node", int(nid), int(n["t"]), int(round(z)), int(round(y)), int(round(x)), -1, -1))
    for a, b in edge_set:
        if a in used and b in used:
            erows.append((-1, ds, "edge", -1, -1, -1, -1, -1, int(a), int(b)))
...
exp_cols = ["id", "dataset", "row_type", "node_id", "t", "z", "y", "x", "source_id", "target_id"]
